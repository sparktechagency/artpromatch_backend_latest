...
exports.BookingService = exports.getUserBookingDetails = void 0;
...
const sendOtpEmailForBookingCompletion_1 = __importDefault(require("../../utils/sendOtpEmailForBookingCompletion"));
...
// create booking
const createBookingIntoDB = async (user, payload) => {
    const session = await (0, mongoose_1.startSession)();
    session.startTransaction();
    try {
        const client = await client_model_1.default.findOne({ auth: user.id }, '_id').session(session);
        if (!client) {
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Client not found!');
        }
        const service = await service_model_1.default.findById(payload.serviceId)
        ...
        const artist = await artist_model_1.default.findById(service.artist)
            .populate('auth', 'email phoneNumber fullName fcmToken')
            .session(session);
        if (!artist) {
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Artist not found!');
        }
        const existingArtistBooking = await booking_model_1.default.findOne({
            artist: artist._id,
            service: service._id,
            status: 'pending',
        }).session(session);
        if (existingArtistBooking) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'You have already booked this service!');
        }
        const existingClientPending = await booking_model_1.default.findOne({
            client: client._id,
            status: 'pending',
        }).session(session);
        if (existingClientPending) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'You already have a pending booking request, you cannot create another until current booking is completed or cancelled!');
        }
        ...
        const booking = await booking_model_1.default.create([bookingPayload], { session });
        if (!booking || !booking[0]) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Failed to create booking!');
        }
        ...
        session.endSession();
        throw new utils_1.AppError(http_status_1.default.INTERNAL_SERVER_ERROR, err.message || 'Booking creation failed');
    }
};
// confirm payment
const confirmPaymentByClient = async (query) => {
    const sessionId = query.sessionId;
    const chSession = await stripe.checkout.sessions.retrieve(sessionId, {
        expand: ['payment_intent'],
    });
    const booking = await booking_model_1.default.findOne({ checkoutSessionId: chSession.id }, 'payment status paymentStatus');
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Session id not found!');
...
// repay booking
const repayBookingIntoDb = async (user, bookingId) => {
    const booking = await booking_model_1.default.findById(bookingId)
        .populate('artist', 'auth')
        .populate('service', 'title description price');
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    const client = await client_model_1.default.findOne({ auth: user.id }, '_id');
    if (!client)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'client not found');
    if (!['pending', 'failed'].includes(booking.paymentStatus) ||
        (booking.paymentStatus === 'pending' &&
            booking.payment.client.paymentIntentId)) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'This booking cannot be repaid');
    }
    const artist = await artist_model_1.default.findById(booking.artist).populate('auth', 'fcmToken');
    if (!artist)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Artist not found');
    const checkoutSession = await stripe.checkout.sessions.create({
        payment_method_types: ['card'],
    ...
    booking.checkoutSessionId = checkoutSession;
    await booking.save();
    return {
...
// get user bookings
const getUserBookings = async (user, query) => {
    ...
    if (user.role === 'CLIENT') {
        ...
        if (!client) {
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Client profile not found');
        }
        ...
        if (!artist) {
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Artist profile not found');
        }
...
// get booking details
const getUserBookingDetails = async (user, bookingId) => {
    ...
    if (user.role === 'CLIENT') {
        ...
        if (booking.client.toString() !== client._id.toString()) {
            throw new utils_1.AppError(http_status_1.default.FORBIDDEN, 'Not authorized to view this booking');
        }
    }
    else if (user.role === 'ARTIST') {
        const artist = await artist_model_1.default.findOne({ auth: user._id });
...
exports.getUserBookingDetails = getUserBookingDetails;
// create Or update Session
const createOrUpdateSessionIntoDB = async (bookingId, payload) => {
    const { sessionId, date, startTime, endTime } = payload;
    const booking = await booking_model_1.default.findById(bookingId);
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    if (booking.status === 'cancelled') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Cannot modify session in a cancelled booking');
    }
    const startTimeInMin = (0, booking_utils_1.parseTimeToMinutes)(startTime);
    const endTimeInMin = (0, booking_utils_1.parseTimeToMinutes)(endTime);
    const duration = endTimeInMin - startTimeInMin;
    if (duration <= 0) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Invalid session duration');
    }
    // EDIT mode
    if (sessionId) {
        const session = booking.sessions.find((s) => s._id?.toString() === sessionId);
        if (!session)
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Session not found');
        if (session.status === 'completed') {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Cannot edit a completed session');
        }
        // update fields
        session.date = date;
    ...
    // CREATE mode
        ...
        const selfOverlap = booking.sessions.find((s) => s.startTimeInMin != null &&
            s.endTimeInMin != null &&
            new Date(s.date).toDateString() === new Date(date).toDateString() &&
            s.startTimeInMin < endTimeInMin &&
            s.endTimeInMin > startTimeInMin);
        if (selfOverlap) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'This booking already has a session during this time');
        }
        // Check overlap with other confirmed or in-progress bookings
        const overlappingBooking = await booking_model_1.default.findOne({
            _id: { $ne: booking._id },
        ...
        if (overlappingBooking) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Artist already has another booking session during this time');
        }
        if (booking.sessions.length > 0) {
            ...
            if (newDate < lastDate ||
                ...
                throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'New session must be scheduled after the previous session');
            }
        }
    ...
    booking.scheduledDurationInMin = booking.sessions.reduce((sum, s) => sum +
        (s.startTimeInMin && s.endTimeInMin
            ? s.endTimeInMin - s.startTimeInMin
            : 0), 0);
    // Recalculate booking status
    const allCompleted = booking.sessions.length > 0 &&
...
// delete session
const deleteSessionFromBooking = async (bookingId, sessionId) => {
    const booking = await booking_model_1.default.findById(bookingId);
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    if (booking.status === 'cancelled') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Cannot delete session from a cancelled booking');
    }
    // Find session
    const sessionIndex = booking.sessions.findIndex((s) => s._id?.toString() === sessionId);
    if (sessionIndex === -1)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Session not found');
    const session = booking.sessions[sessionIndex];
    if (session.status === 'completed') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Cannot delete a completed session');
    }
    // Remove session
...
// get Artist schdule
const getArtistDailySchedule = async (user, date) => {
...
const ReviewAfterAServiceIsCompletedIntoDB = async (payload, clientData) => {
    ...
    try {
        ...
        const secretReview = await secretReview_model_1.default.create([
            {
                service: service._id,
                booking: booking._id,
        ...
        if (!secretReview) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Failed to submit your review!');
        }
        await session.commitTransaction();
...
// confirm booking
const confirmBookingByArtist = async (bookingId) => {
    // 1. Find booking first
    const booking = await booking_model_1.default.findById(bookingId);
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    if (!booking.payment.client.paymentIntentId) {
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'No payment found for this booking');
    }
    if (booking.status !== 'pending') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking cannot be confirmed');
    }
    if (booking.sessions.length === 0) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Cannot confirm booking without at least one session');
    }
    // 2. Capture payment on Stripe (before touching DB)
    const paymentIntent = await stripe.paymentIntents.capture(booking.payment.client.paymentIntentId);
    if (paymentIntent.status !== 'succeeded') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking confirm failed');
    }
    ...
    const updatedBooking = await booking_model_1.default.findOneAndUpdate({ _id: bookingId, status: 'pending' }, // only confirm if still pending
    {
        $set: {
    ...
    if (!updatedBooking) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking is not updated');
    }
    const client = await clientPreferences_model_1.default.findOne({ clientId: booking.client }, 'notificationChannels');
    if (client?.notificationChannels.includes('app')) {
        (0, notification_utils_1.sendNotificationBySocket)({
            ...
            type: notification_constant_1.NOTIFICATION_TYPE.CONFIRMED_BOOKING,
        });
    }
    if (client?.notificationChannels.includes('email')) {
        (0, notification_utils_1.sendNotificationByEmail)(booking.artistInfo.email, notification_constant_1.NOTIFICATION_TYPE.BOOKING_REQUEST, {
    ...
    if (client?.notificationChannels.includes('sms')) {
        const clientDoc = await client_model_1.default.findOne({
            _id: client.clientId,
        }).populate('auth', 'fcmToken');
        if (!clientDoc) {
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'user not found');
        }
        if (clientDoc.auth?.fcmToken) {
            await (0, notification_utils_1.sendPushNotification)(clientDoc.auth.fcmToken, {
...
// complete session
const completeSessionByArtist = async (bookingId, sessionId) => {
    const booking = await booking_model_1.default.findById(bookingId);
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    if (['pending', 'cancelled'].includes(booking.status)) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking is still pending or cancelled');
    }
    const session = booking.sessions.find((s) => s._id?.toString() === sessionId);
    if (!session)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Session not found');
    if (!['scheduled', 'rescheduled'].includes(session.status)) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Session already completed or not schedulable');
    }
    // 🔹 Check previous sessions
    const previousSessions = booking.sessions.filter((s) => s.sessionNumber < session.sessionNumber);
    const incompletePrev = previousSessions.some((s) => s.status !== 'completed');
    if (incompletePrev) {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Previous sessions must be completed first');
    }
...
const artistMarksCompletedIntoDb = async (user, bookingId) => {
    const artist = await artist_model_1.default.findOne({ auth: user.id });
    if (!artist)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Artist not found');
    const booking = await booking_model_1.default.findById(bookingId).populate('client artist status paymentStatus');
    if (!booking)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
    const client = await client_model_1.default.findById(booking.client).populate('auth');
    console.log('client', client);
    if (!client)
        throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'client not found');
    if (booking.paymentStatus !== 'captured') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'payment not found');
    }
    if (booking.status !== 'ready_for_completion') {
        throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'booking is not ready for delivery');
    }
    // Mark artist intent
    // Generate OTP
    ...
    await (0, sendOtpEmailForBookingCompletion_1.default)(client.auth.email, otp, client.auth.fullName);
    return otp;
};
// cancel booking
const cancelBookingIntoDb = async (bookingId, cancelBy) => {
    const session = await (0, mongoose_1.startSession)();
    session.startTransaction();
    try {
        const booking = await booking_model_1.default.findById(bookingId)
        ...
        if (!booking)
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Booking not found');
        if (booking.status === 'cancelled') {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking already cancelled');
        }
        if (!booking.payment.client.paymentIntentId) {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'No payment intent found for this booking');
        }
        // --- Stripe Refund ---
        if (booking.paymentStatus === 'authorized') {
            const cancelPayment = await stripe.paymentIntents.cancel(booking.payment.client.paymentIntentId);
            if (cancelPayment.status !== 'canceled')
                throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'payment not canceled');
        }
        else if (booking.paymentStatus === 'captured') {
            if (cancelBy === 'CLIENT') {
                throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Client cannot cancel this booking! if you need cancel this book pleas contact artist');
            }
            ...
            if (refundAmount > 0) {
                ...
                if (refund.status !== 'succeeded')
                    throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'booking can not be cancelled');
                booking.payment.client.refundId = refund.id;
            }
        }
        else {
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, `Cannot cancel booking in status: ${booking.paymentStatus}`);
        }
        // --- DB Updates ---
...
// complete booking
const completeBookingIntoDb = async (user, bookingId, otp) => {
    const session = await (0, mongoose_1.startSession)();
    session.startTransaction();
    try {
        const booking = await booking_model_1.default.findById(bookingId)
            .select('status paymentStatus payment stripeFee otpExpiresAt service client artist')
            .populate('service', 'totalCompletedOrder')
            .session(session);
        if (!booking)
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'Booking not found');
        const [artist, service] = await Promise.all([
            artist_model_1.default.findOne({ auth: user.id }, 'stripeAccountId totalCompletedService').session(session),
            service_model_1.default.findById(booking.service)
        ...
        if (!service)
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Service not found');
        if (booking.status !== 'ready_for_completion')
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'this booking is not ready for delivery');
        if (booking.paymentStatus !== 'captured')
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'payment is not captured by stripe');
        ...
        if (booking.otpExpiresAt && booking.otpExpiresAt < new Date())
            throw new utils_1.AppError(http_status_1.default.BAD_REQUEST, 'otp time is expired');
        const paymentIntent = await stripe.paymentIntents.retrieve(booking.payment.client.paymentIntentId);
        if (!paymentIntent)
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'no payment found');
        const stripeFee = Math.round(booking.stripeFee * 100);
        const adminPercent = Number(config_1.default.admin_commision) || 5;
        ...
        if (!artist.stripeAccountId)
            throw new utils_1.AppError(http_status_1.default.NOT_FOUND, 'Artist stripe account not found! please open you stripe account');
