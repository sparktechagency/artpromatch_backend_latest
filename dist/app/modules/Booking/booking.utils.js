...
exports.resolveScheduleForDate = exports.roundUpMinutes = exports.minToTimeString = void 0;
exports.parseTimeToMinutes = parseTimeToMinutes;
const guestSpot_model_1 = __importDefault(require("../GuestSpot/guestSpot.model"));
const schedule_model_1 = __importDefault(require("../Schedule/schedule.model"));
const minToTimeString = (min) => {
...
const roundUpMinutes = (min, step = 15) => {
    return Math.ceil(min / step) * step;
};
exports.roundUpMinutes = roundUpMinutes;
...
const resolveScheduleForDate = async (artistId, date) => {
    const dayName = date
        .toLocaleString('en-US', { weekday: 'long', timeZone: 'UTC' })
        .toLowerCase();
    const scheduleDoc = await schedule_model_1.default.findOne({ artistId }).lean();
    if (!scheduleDoc)
        throw new Error('Artist schedule not found');
    // First check guest spot
    if (scheduleDoc.activeGuestSpot) {
        const guestSpot = await guestSpot_model_1.default.findById(scheduleDoc.activeGuestSpot).lean();
        if (guestSpot?.isActive) {
            const dateOnly = new Date(date.getFullYear(), date.getMonth(), date.getDate());
            const gsStartOnly = new Date(guestSpot.startDate.getFullYear(), guestSpot.startDate.getMonth(), guestSpot.startDate.getDate());
            const gsEndOnly = new Date(guestSpot.endDate.getFullYear(), guestSpot.endDate.getMonth(), guestSpot.endDate.getDate());
...
exports.resolveScheduleForDate = resolveScheduleForDate;
